...
export default function SubscriptionStatus({ onReloadSubscription }: SubscriptionStatusProps) {
    ...
    const fetchSubscription = async () => {
        try {
            ...
            setError('サブスクリプション情報を読み込めませんでした');
    ...
    const handleReactivate = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.error || 'サブスクリプションの再アクティブ化に失敗しました');
            }
            ...
            // 結果を反映
            setSubscription(data.subscription);
            toast.success('サブスクリプションを再アクティブ化しました');
            ...
            toast.error(err instanceof Error ? err.message : 'サブスクリプションの再アクティブ化に失敗しました');
        } finally {
    ...
    const handleCancel = async () => {
        ...
        if (!window.confirm('サブスクリプションをキャンセルしてもよろしいですか？\n\n現在の期間が終了するまではご利用いただけます。')) {
            return;
        }
        ...
        return (
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 flex items-center justify-center min-h-[200px]">
                <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">サブスクリプション情報を読み込んでいます...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
            ...
            {subscription && 'isMockData' in subscription && subscription.isMockData && (
                <div className="mb-4 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
                    開発用のデモデータが表示されています。実際のサブスクリプションデータではありません。
                </div>
            )}
            <motion.div
                ...
                <div className="flex items-start">
                    ...
                    <div className="flex-1">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                            <h3 className="text-lg font-medium mb-2 sm:mb-0">サブスクリプション状態</h3>
                            <span className={`text-sm font-medium px-2 py-1 rounded-full ${statusDisplay.className} inline-block`}>
                                {statusDisplay.text}
                            </span>
                        </div>
                                ...
                                <span className="text-sm font-medium">
                                    {subscription.plan === 'monthly' ? '月額プラン' :
                                        subscription.plan === 'yearly' ? '年額プラン' :
                                            subscription.plan === 'business' ? '法人プラン' :
                                                subscription.plan}
                            ...
                            {subscription.status === 'trialing' && (
                                <div className="bg-blue-50 border border-blue-100 rounded-md p-3 mt-4">
                                    <p className="text-sm text-blue-800 text-justify">
                                        無料トライアル期間中です。
                                        <strong>{formatDate(subscription.trialEnd || subscription.currentPeriodEnd)}</strong>
                                        まで{subscription.plan === 'monthly' ? '月額' : subscription.plan === 'yearly' ? '年額' : ''}プランをお試しいただけます。
                                    </p>
                                </div>
                            )}
