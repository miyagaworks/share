// components/touch-seal/StripeCheckout.tsx
'use client';
...
function CheckoutForm({ orderId, amount, onSuccess, onCancel }: StripeCheckoutProps) {
  ...
  useEffect(() => {
    const createPaymentIntent = async () => {
      try {
        const response = await fetch('/api/touch-seal/create-payment-intent', {
          method: 'POST',
  ...
  const canSubmit = stripe && elements && clientSecret && !isProcessing;

  return (
    <Card className="p-6 max-w-md mx-auto">
      <div className="flex items-center mb-4">
        <CreditCard className="h-5 w-5 mr-2 text-blue-600" />
        <h3 className="text-lg font-semibold">お支払い情報</h3>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        {/* カード入力フィールド */}
        <div className="mb-4">
          <label className="block text-sm font-medium mb-2">クレジットカード情報</label>
        ...
        <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center text-green-800">
            ...
            <span className="text-sm font-medium">セキュアな決済</span>
          </div>
          <p className="text-xs text-green-700 mt-1">Stripeにより暗号化されて安全に処理されます</p>
        </div>

        {/* 決済ボタン */}
        <div className="space-y-3">
          <Button type="submit" disabled={!canSubmit} className="w-full h-12" size="lg">
            {isProcessing ? (
              <>
...
export function PaymentSuccess({ orderId, amount, onContinue }: PaymentSuccessProps) {
  return (
