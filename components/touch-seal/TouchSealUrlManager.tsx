// components/touch-seal/TouchSealUrlManager.tsx
'use client';
...
interface TouchSealUrlManagerProps {
  qrSlug: string;
  onQrSlugChange: (slug: string) => void;
  disabled?: boolean;
  userQrSlug?: string; // 既存のユーザーQRスラッグ
  userName?: string;
}

export function TouchSealUrlManager({
  qrSlug,
  onQrSlugChange,
  disabled = false,
  userQrSlug,
  userName,
}: TouchSealUrlManagerProps) {
  ...
  const validateSlug = useCallback(async (slug: string) => {
    ...
    setIsChecking(true);
    try {
      const response = await fetch(`/api/touch-seal/validate-qr?slug=${encodeURIComponent(slug)}`);
