...
export async function processWithIdempotency<T>(
  ...
  // 既存の結果がある場合はそれを返す
  if (existingRequest) {
    logger.debug(`処理済みリクエスト: ${idempotencyKey} (${endpoint})`);
  ...
  try {
    ...
    logger.error('冪等性リクエスト保存エラー:', error);
...
export async function cleanUpExpiredIdempotencyRequests() {
  ...
  try {
    const result = await prisma.$executeRaw`
      ...
      WHERE "expiresAt" < ${now}
    `;
    logger.debug(`期限切れ冪等性リクエスト削除: ${result}件`);
    return Number(result);
  } catch (error) {
    logger.error('期限切れ冪等性リクエスト削除エラー:', error);
    return 0;
  }
}
/**
