// app/auth/signin/SigninClient.tsx (パートナーブランド対応版)
'use client';
...
export interface SigninBrandProps {
  brandName: string;
  tagline: string;
  logoUrl: string | null;
...
/**
 * CSS色をグラデーションに変換するヘルパー
 * primaryColor からデコレーション部分の背景グラデーションを生成
 */
function buildGradientStyle(primaryColor: string) {
  // デフォルトのblue系なら従来通り
  if (primaryColor === '#3B82F6' || primaryColor === '#2563eb') {
    return {
      background: 'linear-gradient(135deg, #2563eb 0%, #1e40af 100%)',
      overlay: '#1d4ed8',
    };
  }
  // パートナーカラーの場合、primaryColor ベースでグラデーション
  return {
...
export default function SigninClient({ brand }: { brand: SigninBrandProps }) {
  const { brandName, tagline, logoUrl, logoWidth, logoHeight, primaryColor, isPartner, isBuyout } = brand;
  const gradient = buildGradientStyle(primaryColor);
  ...
  // ボタンのブランドカラースタイル
  const submitButtonActiveClass = isPartner
    ? ''
    : 'bg-blue-600 hover:bg-blue-800';
  const submitButtonInactiveClass = isPartner
    ? ''
    : 'bg-blue-300 hover:bg-blue-400';
  const submitButtonActiveStyle = isPartner
    ? { backgroundColor: primaryColor, opacity: 1 }
    : {};
  const submitButtonInactiveStyle = isPartner
    ? { backgroundColor: primaryColor, opacity: 0.5 }
    : {};

  // リンクのブランドカラー
  const linkColorClass = isPartner ? '' : 'text-blue-600 hover:text-blue-500';
  const linkColorStyle = isPartner ? { color: primaryColor } : {};
  ...
  return (
    <div className="flex min-h-screen">
      ...
      >
        <div className="absolute inset-0 opacity-20" style={{ backgroundColor: gradient.overlay }}>
          ...
          <div className="flex flex-col space-y-4 mt-12">
            ...
            >
              <p className="text-left mb-3" style={{ color: '#ffffff' }}>
                {`「${brandName}」を使えば、あなたのSNSアカウントと連絡先情報をひとつにまとめて、簡単に共有できます。`}
              </p>
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="md:hidden flex justify-center mb-8">
              {isPartner && logoUrl ? (
                ...
                <Image src="/logo_blue.svg" alt={`${brandName} Logo`} width={90} height={90} priority />
              )}
            </div>
          ...
          <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex justify-end mt-1">
                      <Link
                        ...
                        className={`text-sm hover:underline ${linkColorClass}`}
                  ...
                  <div>
                    <Button
                      ...
                      className={`w-full text-white transition-colors shadow-md min-h-[48px] md:min-h-0 ${
                        isFormValid
                          ? `${submitButtonActiveClass} transform hover:-translate-y-0.5`
                          : submitButtonInactiveClass
                      }`}
                      style={isFormValid ? submitButtonActiveStyle : submitButtonInactiveStyle}
            ...
            <div className="text-center text-sm mt-8">
              ...
              <Link
                href="/auth/signup"
                className={`font-medium hover:underline ${linkColorClass}`}
                style={linkColorStyle}
              >
                新規登録
              </Link>{' '}
