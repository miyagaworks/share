...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    logger.debug(`[API] /api/corporate/users/${params.id} PATCHリクエスト受信`);
    // 永久利用権ユーザーかどうかチェック
    ...
    const body = await request.json();
    const { role, departmentId } = body;
    logger.debug(`[API] 更新対象ID: ${params.id}, 新しい役割: ${role}, 部署ID: ${departmentId}`);
    // 対象ユーザーの現在の情報を取得
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    logger.debug(`[API] /api/corporate/users/${params.id} DELETEリクエスト受信`);
    // 永久利用権ユーザーかどうかチェック
    const isPermanent = checkPermanentAccess();
    if (isPermanent) {
      return NextResponse.json({
