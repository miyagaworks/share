// app/api/admin/touch-seal/orders/[id]/route.ts
...
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    // 注文の存在確認
    const existingOrder = await prisma.touchSealOrder.findUnique({
      where: { id: orderId },
      include: {
        user: {
          select: {
    ...
    // 注文を更新
    const updatedOrder = await prisma.touchSealOrder.update({
      where: { id: orderId },
      data: updateData,
      include: {
        user: {
    ...
    logger.error('タッチシール注文更新エラー:', error);
