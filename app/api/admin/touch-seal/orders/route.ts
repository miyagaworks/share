// app/api/admin/touch-seal/orders/route.ts
export const dynamic = 'force-dynamic';
import { NextResponse } from 'next/server';
import { logger } from '@/lib/utils/logger';
import { auth } from '@/auth';
import { prisma } from '@/lib/prisma';
import { TouchSealOrder } from '@/types/touch-seal';
...
export async function GET() {
  try {
    ...
    // 全てのタッチシール注文を取得
    const orders = await prisma.touchSealOrder.findMany({
      include: {
        items: {
          include: {
            memberUser: {
    ...
    // 管理者用の形式で整形（TouchSealOrder型を使用）
    const formattedOrders: TouchSealOrder[] = orders.map((order) => ({
      id: order.id,
      userId: order.userId,
      ...
      status: order.status as TouchSealOrder['status'],
    ...
    logger.error('管理者用タッチシール注文取得エラー:', error);
