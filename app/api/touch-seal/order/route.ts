// app/api/touch-seal/order/route.ts
...
import { z } from 'zod';
import { TOUCH_SEAL_COLORS, TOUCH_SEAL_CONFIG, type CreateTouchSealItem } from '@/types/touch-seal';
import { calculateOrderAmount, validateTouchSealOrder } from '@/lib/touch-seal/order-calculator';
import { validatePostalCode } from '@/lib/touch-seal/qr-slug-manager';
...
const CreateOrderSchema = z.object({
  ...
  items: z.array(
    z.object({
      color: z.enum(TOUCH_SEAL_COLORS),
      quantity: z.number().min(1).max(TOUCH_SEAL_CONFIG.MAX_QUANTITY_PER_COLOR),
...
export async function POST(request: Request) {
  try {
    ...
    // ValidationTouchSealItem型に変換してバリデーション
    const validationItems = orderData.items.map((item) => ({
      ...item,
      unitPrice: TOUCH_SEAL_CONFIG.UNIT_PRICE,
    }));

    const itemValidation = validateTouchSealOrder(validationItems);
    ...
    const result = await prisma.$transaction(async (tx) => {
      // 注文作成
      const order = await tx.touchSealOrder.create({
        data: {
    ...
    logger.error('タッチシール注文作成エラー:', error);
