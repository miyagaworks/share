...
async function sendMultipartEmail({
  ...
  textContent,
  htmlContent,
  from = process.env.MAIL_FROM || 'noreply@sns-share.com',
}: SendEmailParams) {
  // メール送信用のトランスポーターを設定
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  const mail = {
    ...
    to,
    subject,
    text: `${textContent}${textSignature}`,
    html: `${htmlContent}${htmlSignature}`,
  };
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  <div style="background-color: #fef2f2; padding: 12px; border-radius: 4px; border-left: 4px solid #ef4444;">
    <p style="margin: 0; color: #b91c1c; font-weight: 500;">※このお問い合わせは優先対応が必要です。1営業日以内に返信してください。</p>
      ...
      // 自動返信用のメール本文（テキスト形式）
      const autoReplyTextContent = `
${name} 様

法人プランへのお問い合わせありがとうございます。
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  <p style="color: #6B7280; font-size: 13px; margin-top: 30px; padding-top: 10px; border-top: 1px solid #e2e8f0;">
    ※このメールは自動送信されています。このメールには返信しないでください。
      ...
      await sendMultipartEmail({
        to: email,
        subject: `【Share】法人プランへのお問い合わせを受け付けました`,
        textContent: autoReplyTextContent,
        htmlContent: autoReplyHtmlContent,
      });
    } else {
      // 通常のお問い合わせ処理
      // 管理者向けメール本文（テキスト形式）
