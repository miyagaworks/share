...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log('トライアル通知処理を開始します');

    // トライアル終了通知メールを送信
    const trialResult = await sendTrialEndingEmails();
    console.log('トライアル終了通知処理が完了しました', trialResult);

    // 猶予期間終了ユーザーをチェックして管理者に通知
    console.log('猶予期間終了チェックを開始します');
    const graceResult = await checkExpiredGracePeriods();
    console.log('猶予期間終了チェックが完了しました', graceResult);
    ...
    console.error('トライアル通知スケジュールエラー:', error);
    return NextResponse.json(
      {
