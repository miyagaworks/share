// app/api/one-tap-seal/create-payment-intent/route.ts
    ...
    try {
      const paymentIntent = await stripe.paymentIntents.create({
        ...
        metadata: {
          orderId: order.id,
          userId: session.user.id,
          orderType: 'one_tap_seal',
          itemCount: order.items.reduce((sum, item) => sum + item.quantity, 0).toString(),
        },
        description: `ワンタップシール注文 - ${order.items.reduce((sum, item) => sum + item.quantity, 0)}枚`,
      ...
      logger.info('ワンタップシール PaymentIntent作成完了', {
        orderId,
      ...
      logger.error('ワンタップシール PaymentIntent creation failed:', stripeError);
      return NextResponse.json({ error: '決済の準備に失敗しました' }, { status: 500 });
    }
  } catch (error) {
    logger.error('ワンタップシール PaymentIntent作成エラー:', error);
    return NextResponse.json({ error: '決済の準備に失敗しました' }, { status: 500 });
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      logger.error('ワンタップシール PaymentIntent retrieval failed:', stripeError);
      return NextResponse.json({ error: '決済状況の確認に失敗しました' }, { status: 500 });
    }
  } catch (error) {
    logger.error('ワンタップシール PaymentIntent確認エラー:', error);
    return NextResponse.json({ error: '決済状況の確認に失敗しました' }, { status: 500 });
  }
}